...
use crate::{from::From, grammar::{self, SelectList, TableExpression, TableReferenceList}, ToQuery};
...
/// See [self::select]
pub struct BeginSelect<SeLs> where SeLs: SelectList {
    select_list: SeLs
}

impl<SeLs> BeginSelect<SeLs> where SeLs: SelectList {
    pub fn from<TabRefs: TableReferenceList>(self, table_refs: TabRefs) 
...
where SeLs: SelectList, TabExpr: TableExpression 
{
    #[inline]
    ...
    #[inline]   
    fn and_from(self, table_refs: impl TableReferenceList) -> impl grammar::Select {
        Select {
            quantifier: self.quantifier,
            select_list: self.select_list,
            table_expression: self.table_expression.and_from(table_refs)
        }
    }
}

impl<SeLs, TabExpr> ToQuery for Select<SeLs, TabExpr> where SeLs: SelectList, TabExpr: TableExpression {
...
mod tests {
    use crate::{
        grammar::{
            DerivedColumn, Select, SelectList
        }, 
        identifier::id, 
        select::select, ToQuery as _
    };
    ...
    fn test_select_basic() {
        ...
        let stmt = select(selected_columns)
            .from(id("my_table"));
